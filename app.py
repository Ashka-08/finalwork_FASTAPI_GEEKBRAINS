...
# REST API Методы для CRUD операций с пользователями
...
# REST API Методы для CRUD операций с товарами
...
async def update_item(item_id: int, new_item: Items):
    query = items.update().where(items.c.item_id == item_id).values(
...
# REST API Методы для CRUD операций с заказами
